...
function HomeFilterModal({ initialTab, initialValues, onApply, dismiss }: HomeFilterModalProps) {
  ...
  const handleClear = () => {
    if (activeTab === "type") setLocalType(null);
    if (activeTab === "price") { setLocalMinPrice(""); setLocalMaxPrice(""); }
    if (activeTab === "bedrooms") setLocalBedrooms(null);
  };
          ...
          <div className="grid grid-cols-2 gap-2">
            {HOME_HERO_FILTER_OPTIONS.types.map((o) => (
              <button
                key={o.value}
                type="button"
                onClick={() => setLocalType(localType === o.value ? null : o.value)}
                className={classNames(
                  "rounded-xl border-2 px-4 py-2.5 text-left text-sm font-semibold transition-all",
                  localType === o.value
          ...
          <div className="grid grid-cols-3 gap-2">
            {HOME_HERO_FILTER_OPTIONS.bedrooms.map((o) => (
              <button
                key={o.value}
                type="button"
                onClick={() => setLocalBedrooms(localBedrooms === o.value ? null : o.value)}
                className={classNames(
                  "rounded-xl border-2 px-4 py-2.5 text-sm font-semibold transition-all",
                  localBedrooms === o.value
                    ? "border-primary-700 bg-primary-50 text-primary-700"
                    : "border-gray-200 bg-white text-gray-700 hover:border-gray-300",
                )}
              >
